...
type TSymbols = keyof typeof symbolData
...
let RefLabel = (props: any) => {
    const { y } = props.viewBox
    return (
        <text x={'50%'} y={y} fill={'blue'} dy={-4} fontSize={30} textAnchor='middle'>
            Stop Loss
        </text>
        // <div
        //     id='label'
...
const Home: NextPage = () => {
    ...
    const [symbol, setSymbol] = useState<TSymbols>('LTCUSDT')
        ...
        if (!loaded && data.length) {
            ...
            let fn = async () => {
                for (let index = 0; index < 100; index++) {
                    ...
                    if ((endTimestamp - startTimestamp) / 6000 > 60) {
                        break
                    }
                    await sleep(10)
                }
